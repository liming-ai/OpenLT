    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True):
        ...
        if balanced:
            if training:
                ...
                shuffle = False
                sampler = ClassAwareSampler(dataset, 4)
            else:
...
class TinyImageNetLTDataLoader(DataLoader):
    """
    TinyImageNetLT Data Loader
    """
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True):
        train_trsfm = transforms.Compose([
            transforms.RandomResizedCrop(224),
        ...
        if training:
            dataset = LT_Dataset(data_dir, data_dir + '/Tiny_ImageNet_LT_train.txt', train_trsfm)
            val_dataset = LT_Dataset(data_dir, data_dir + '/Tiny_ImageNet_LT_val.txt', test_trsfm)
        else: # test
            dataset = LT_Dataset(data_dir, data_dir + '/Tiny_ImageNet_LT_test.txt', test_trsfm)
            val_dataset = None
