...
from utils import load_state_dict, rename_parallel_state_dict, rename_classifier_state_dict
import shutil
...
class BaseTrainer:
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, lr_scheduler, config):
        ...
        if 'val_start_epoch' in cfg_trainer:
            self.val_start_epoch = cfg_trainer['val_start_epoch']
        ...
        # copy core code
        shutil.copyfile('train.py', str(config.code_dir) + '/train.py')
        shutil.copyfile('model/loss.py', str(config.code_dir) + '/loss.py')
        shutil.copyfile('trainer/trainer.py', str(config.code_dir) + '/trainer.py')

        # setup visualization writer instance
        self.writer = TensorboardWriter(config.log_dir, self.logger, cfg_trainer['tensorboard'])

        if config.load_crt is not None:
            self.logger.info("Loading from cRT pretrain: {}".format(config.load_crt))
            self._load_crt(config.load_crt)

        if config.load_lws is not None:
            self.logger.info("Loading from LWS pretrain: {}".format(config.load_lws))
            self._load_lws(config.load_lws)

        if config.load_pretrain is not None:
            self.logger.info("Loading from pretrained model: {}".format(config.load_pretrain))
    ...
    def _load_crt(self, cRT_pretrain):
        ...
        for name, param in self.real_model.named_parameters():
            if not name.startswith('model.classifier'):
                param.requires_grad_(False)
            else:
                print("Allow gradient on:", name)
        print("** Please check the list of allowed gradient to confirm **")

    def _load_lws(self, LWS_pretrain):
        """
        Load from LWS pretrain
        :param LWS pretrain path to the checkpoint of LWS pretrain
        """
        state_dict = torch.load(LWS_pretrain)['state_dict']
        rename_parallel_state_dict(state_dict)
        rename_classifier_state_dict(state_dict)

        load_state_dict(self.real_model, state_dict)

        for name, param in self.real_model.named_parameters():
            if not name.startswith('model.classifier.learned'):
                param.requires_grad_(False)
            else:
                print("Allow gradient on:", name)
    ...
    def _load_pretrain(self, pretrained_model):
        checkpoint = torch.load(pretrained_model)
        state_dict = checkpoint['state_dict']

        if 'moco' in pretrained_model:
            for k in list(state_dict.keys()):
                # retain only encoder_q up to before the embedding layer
                if k.startswith('module.encoder_q') and not k.startswith('module.encoder_q.fc'):
                    # remove prefix
                    state_dict[k.replace('encoder_q', 'model')] = state_dict[k]
            ...
            raise Exception('Only support MoCo pretrained model for now')
    ...
    def _resume_checkpoint(self, resume_path, state_dict_only=False):
        ...
        if not state_dict_only:
            ...
            if 'lr_scheduler' in checkpoint:
                ...
                self.logger.info("lr_scheduler state dict is loaded")
            else:
                self.logger.info("lr_scheduler state dict is not found, so it's not loaded.")
