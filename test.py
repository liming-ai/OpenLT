...
from utils import calibration, reliability_diagram
...
def main(config):
    ...
    if hasattr(model, "confidence_model") and model.confidence_model:
        cumulative_sample_num_experts = torch.zeros((model.model.num_experts, ), dtype=torch.float, device=device)
        cumulative_sample_num_experts_each_shot = [torch.zeros((model.model.num_experts, ), dtype=torch.float, device=device) for _ in range(3)]
        num_samples = 0
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(tqdm(data_loader)):
            ...
            # loss = loss_fn(output, target, classifier_weight, feat)
    ...
    cal = reliability_diagram(true_class, pred_class, confidence, num_bins=15, title='Ours', save_fig=False)
    print('ECE: ', cal["expected_calibration_error"])
